...
const ViewDetails = () => {
  ...
  const shareViaEmail = (item) => {
    ...
    const body = `  ${imageUrl} Hi, I found this amazing product: ${item.dishes}. It costs AED ${item.price}. Check it out at ${window.location.href}`;
  ...
  const shareViaWhatsApp = (item) => {
    ...
    const imageUrl = `${process.env.REACT_APP_MACHINE_TEST_1_BACKEND_URL}/images/${item.image[currentImageIndex]}`;
    const message = ` ${imageUrl} Check out this product: ${item.dishes}. It costs AED ${item.price}. Check it out at ${window.location.href}. `;
