...
const DishesPage = () => {
    ...
    const redirectToWhatsApp = (order) => {
      ...
      const message = `Product: ${imageUrl} Hi, I'd like to order ${order.dishes} for AED ${order.price}. You can view the product here: ${productUrl}`;
    ...
    //   const phoneNumber = '+971585023411';
    //   const imageUrl = `${process.env.REACT_APP_MACHINE_TEST_1_BACKEND_URL}/images/${order.image[currentImageIndex]}`;
    //   const message = `Product: ${imageUrl} Hi, I'd like to order  ${order.dishes} for AED ${order.price}.`;
    //   const encodedMessage = encodeURIComponent(message);
    //   const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
    //   window.open(whatsappUrl, '_blank');
    // };
