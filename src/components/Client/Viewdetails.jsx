...
import { Row, Col, Button, Carousel, Dropdown } from 'react-bootstrap';
...
const ViewDetails = () => {
  ...
  const shareViaEmail = (order) => {
    const imageUrl = `${process.env.REACT_APP_MACHINE_TEST_1_BACKEND_URL}/images/${order.image[currentImageIndex]}`;
    const subject = `Check out this product: ${order.dishes}`;
    const body = `  ${imageUrl} Hi, I found this amazing product: ${order.dishes}. It costs AED ${order.price}. Check it out at ${window.location.href}`;
    window.location.href = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  };

  const shareViaWhatsApp = (order) => {
    const imageUrl = `${process.env.REACT_APP_MACHINE_TEST_1_BACKEND_URL}/images/${order.image[currentImageIndex]}`;
    const productUrl = `${window.location.origin}/view-details/${order.id}`; // Ensure the correct URL format
    const message = `Check out this product: ${order.dishes}. It costs AED ${order.price}. \n${imageUrl} \nCheck it out at ${productUrl}.`;
                              ...
                              <div className="d-flex align-items-center justify-content-center">
                                <div style={{ width: '342px', height: '513px' }}>
                                  <ReactImageMagnify
                                    ...
                                    onActivationChanged={setIsZoomed}
                                    style={{
                                      cursor: 'pointer',
            ...
            <Col md={6} className="d-flex align-items-center justify-content-center">
              {item && (
                <div>
                  ...
                  <h2 style={{color:'#b2b20a'}}><b>{item.dishes}</b></h2>
                  <hr className="divider" />
                  <h6>{item.description}</h6>
                  <h4>Price: AED {item.price}</h4>
...
//                             <Carousel.Item key={index}>
//                               <Col md={6} className="d-flex align-items-center justify-content-center">
//                                 <div className='bg-red-400' style={{ width: '342px', height: '513px' }}>
//                                   <ReactImageMagnify
//                                     {...{
//                                       smallImage: {
//                                         alt: `Image ${index + 1}`,
...
//                                     alt: item.dishes,
//                                     isFluidWidth: true,
//                                     src: `${process.env.REACT_APP_MACHINE_TEST_1_BACKEND_URL}/images/${item.image[currentImageIndex]}`,
//                                   },
//                                   largeImage: {
//                                     src: `${process.env.REACT_APP_MACHINE_TEST_1_BACKEND_URL}/images/${item.image[currentImageIndex]}`,
//                                     width: 200,
...
//             {/* VIDEO BUTTON */}
//             {/* <div>
//               <Button variant="dark" onClick={handleModalOpen}>Show Similar</Button>
//             </div> */}
...
//                 <source src="https://youtu.be/-_SOBHfaqNw?si=mwDY5M_lhh12-SUI" type="video/mp4" />
//               </video>
//             </div>
//           </Modal.Body>
//         </Modal> */}
